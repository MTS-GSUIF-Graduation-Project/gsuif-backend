...
| **BE-09** Global Exception Handling | Jmix 3.0.1 / OpenAPI 7.16.0 | ❌ / ❌ | ❌ / ❌ | — | — | — | **Hand-written** | Neither tool generates `@RestControllerAdvice`. `GlobalExceptionHandler` is hand-written per STD-09–13 and STD-28 / DEC-023: 400 (malformed/invalid request), 401, 403, 404, 405 (`HttpRequestMethodNotSupportedException`), 415 (`HttpMediaTypeNotSupportedException`), and 500, all in the five-field `ApiResponse` envelope. |
...
| **BE-11** API Documentation (Swagger/OpenAPI) | OpenAPI 7.16.0 | ⚠️ | ⚠️ | ✅ (reproducible) | ⚠️ partial | none beyond springdoc | **OpenAPI Generator (annotations only) + Hand-written** | OpenAPI generates `@Operation`, `@ApiResponse`, `@Parameter` annotations from YAML `responses:` blocks (STD-27 partial). The full STD-28 / DEC-023 baseline (200/201/400/401/403/404/405/415/500) must be declared in the spec when applicable; 405 and 415 are protocol-level and are closed by hand-written SpringDoc coverage plus `GlobalExceptionHandler` (BE-09). SpringDoc bean config, SecurityConfig Swagger integration, and `@Schema` on framework DTOs are hand-written. Component scope is not fully covered — marked partial. |
...
| STD-28 | HTTP status codes — DEC-023 semantic baseline (200/201/400/401/403/404/405/415/500; other documented codes allowed; every error uses `ApiResponse`) | ⚠️ partial | Spike YAML declares **200**, **201**, and **404** only. DELETE uses **200** (not 204), which still matches STD-28 + the `ApiResponse` body requirement. It does **not** declare 400/401/403/405/415/500. OpenAPI Generator can emit those codes if they are added to the spec, but 405 and 415 are Spring MVC protocol responses closed by hand-written `GlobalExceptionHandler` (BE-09), not by the generator. |
