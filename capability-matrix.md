...
> **Decisions:** DEC-021 (Jmix REJECT) · DEC-022 (OpenAPI ACCEPT WITH LIMITATIONS) · DEC-023 (STD-28 semantic HTTP baseline)
...
| **BE-09** Global Exception Handling | Jmix 3.0.1 / OpenAPI 7.16.0 | ❌ / ❌ | ❌ / ❌ | — | — | — | **Hand-written** | Neither tool generates exception or security response infrastructure. Hand-written split: `GlobalExceptionHandler` (`@RestControllerAdvice`, BE-09) maps application/MVC errors per STD-09–13 and STD-28 / DEC-023 — 400 (malformed/invalid request), 404, 405 (`HttpRequestMethodNotSupportedException`), 415 (`HttpMediaTypeNotSupportedException`), and 500, all in the five-field `ApiResponse` envelope. `JwtAuthenticationEntryPoint` and `JwtAccessDeniedHandler` (BE-06 / `SecurityConfig`) produce 401 and 403 in the same envelope. |
...
| **BE-11** API Documentation (Swagger/OpenAPI) | OpenAPI 7.16.0 | ⚠️ | ⚠️ | ✅ (reproducible) | ⚠️ partial | none beyond springdoc | **OpenAPI Generator (annotations only) + Hand-written** | OpenAPI Generator emits `@Operation`, `@ApiResponse`, and `@Parameter` from YAML `responses:` blocks (STD-27 partial). The STD-28 / DEC-023 baseline must be declared per endpoint when applicable. Runtime 405/415 responses are handled by `GlobalExceptionHandler` (BE-09), but OpenAPI/SpringDoc documentation for 405/415 and the full baseline on framework endpoints is not yet implemented — remaining hand-written gap for SCRUM-29 (T-20). SpringDoc bean config, SecurityConfig Swagger integration, and `@Schema` on framework DTOs are hand-written. Component scope is not fully covered — marked partial. |
...
| STD-28 | HTTP status codes — DEC-023 semantic baseline (200/201/400/401/403/404/405/415/500; other documented codes allowed; every error uses `ApiResponse`) | ⚠️ partial | Spike YAML declares **200**, **201**, and **404** only. DELETE uses **200** (not 204). Runtime 405/415 are implemented in `GlobalExceptionHandler` (verified by tests), but the spike spec does not document them. OpenAPI Generator can emit additional codes only if declared in the spec; full STD-28 OpenAPI coverage remains a SCRUM-29 (T-20) gap. |
